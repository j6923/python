...
for j in range(1,21):
    ...
    FOOD_CATEGORY = store1.find_element_by_css_selector(".category").text
    ADDRESS = store1.find_elements_by_css_selector("div > div > div.txt.address").TEXT
...
def save(self,NAME,FOOD_CATEGORY,ADDRESS,REVIEW):
    connection1 = cx_Oracle("oracle","scott","192.168.0.32:1521/orcl")
    print(connection1)
    cur = connection.cursor()

    sql = """
    INSERT INTO NAME VALUES(:NAME, :FOOD_CATEGORY, :ADDRESS, :REVIEW)
    """
    #실행
    cur.execute(sql,NAME= NAME , FOOD_CATEGORY= FOOD_CATEGORY, ADDRESS= ADDRESS, REVIEW= REVIEW)
    connection.commit()
    #자원반납 
    connection.close()
    connection1.close()
