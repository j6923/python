...
def save(NAME,categoryName,ADDRESS,REVIEW):
    print("save함수 호출중")
    print("--------------------------------------------------------------------------------")

    print(NAME,categoryName,ADDRESS,REVIEW)
    print("--------------------------------------------------------------------------------")
    ...
    sql = """
    INSERT INTO FOODSS VALUES(:NAME, :FOOD_CATEGORY, :ADDRESS, :REVIEW)
    """
    #실행
    cur.execute(sql,NAME= NAME , FOOD_CATEGORY= categoryName, ADDRESS= ADDRESS, REVIEW= REVIEW)
    connection.commit()
    #자원반납 
...
def dbCheck(self):
    print("저장되었나?")
    connection = cx_Oracle("oralce", "scott","192.168.0.32:1521/orcl")
    cur = connection.cursor()
    sql = """
    SELECT NAME, FOOD_CATEGORY,ADDRESS,REVIEW
    FROM FOOD
    """
    ...
    for store1 in lists1:
        ...
        try : 
            store1.find_element_by_css_selector(".category")
                ...
                # 카테고리가 존재 
            NAME = store1.find_element_by_css_selector("div.tit > span > a > span").text   
            categoryName = store1.find_element_by_css_selector(".category").text
            ADDRESS = store1.find_element_by_css_selector("div > div > div.txt.address").text
            REVIEW = store1.find_element_by_css_selector(".item").text
            ...
            if categoryName != "주점" and categoryName != "호프" and categoryName != "맥주" and categoryName != "다방" and categoryName != "맥주,호프":
                ...
                print("-------------------------------------------------------------------------------------------------------")
                print(NAME,categoryName,ADDRESS,REVIEW)
                save(NAME,categoryName,ADDRESS,REVIEW)
        except:
            pass
        ...
        pyautogui.click(763,613)
        #pyautogui.click(763,612)
        # resultList=[]
