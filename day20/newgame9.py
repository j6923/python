...
import math         #math을 불러온다. 
import random       #random을 불러온다. 

#초기화 하기전 초기화 해줘야함. 
pygame.init()   #pygame을 초기화한다. 
#미사일과 충돌 테스트 
def pythagoras(x1,y1,x2,y2): #pythagoras라는 함수를 만들고 x1,y1,x2,y2를 매게변수로 한다. 
    return math.sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)) #x측y측간의 차이의 제곱 #자꾸 불러쓰면 소스가 엉키게 됨.
#math의 sqrt를 하는데 ((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1))를 한다. 

def makeEnemy(): #makeEnemy라는 함수를 만든다. 
...
def collision(x1,y1, x2,y2):       
    ...
    result = 0 #?? 
    if dis < 30:  #만약 dis(피타고라스) 30보다 작으면 
        result =1    #result에 -1을 한다. 
    return result    #result값을 리턴한다. 

#가까우면 충돌 안 가까우면 충돌 X 

screen_width = 600  #너비 높임 
screen_height = 900 #높이 900을 screen_height에 대입. 
