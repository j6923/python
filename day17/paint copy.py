...
class MyApp(QWidget): #얘를 상속받은 클래스 됨, 나도 위잿이 될 수 있음. 
    ...
    def initUI(self):  #Ui관련된것 
        ...
        self.rbtnLine = QRadioButton("직선",self) #이름이 있어야 되니까 
...
#지우개 사각형해서 흰색으로 하면 됨. 
        ...
        self.rbtnLine.clicked.connect(self.radioBtnClicked) #클릭하는 시그널이 오면 처리하기 위해 rado호출 , 함수호출한다. 
        self.rbtnCurve.clicked.connect(self.radioBtnClicked) #클릭하는 시그널이 오면 처리하기 위해 rado호출 , 함수호출한다. 
        self.rbtnRect.clicked.connect(self.radioBtnClicked) #클릭하는 시그널이 오면 처리하기 위해 rado호출 , 함수호출한다. 
        self.rbtnELLipse.clicked.connect(self.radioBtnClicked) #클릭하는 시그널이 오면 처리하기 위해 rado호출 , 함수호출한다. 
        #
    ...
    def radioBtnClicked(self):
        print("사각형 라디오 버튼 클릭됨")
        #어떤 라디오 버튼이 선택된 것인지 판단
        if self.rbtnLine.isChecked(): #이 버튼이 선택되었다면
            self.drawType = 1
...
#시작점 끝점 동일함  --- 직선과 사각형 
    ...
    def mouseReleaseEvent(self,e): #마우스 떼는 것을 release라고 함.
        ...
        if self.parent().drawType == 1:
            ...
            # pen=QPen()
            self.scene.addLine(line,pen)
        elif self.parent().drawType ==2:  #부모의 drawType이 2라면 
            brush = QBrush(self.parent().brushcolor)   #부모가 한 브러쉬색깔을 Q브러쉬로 하고 그것을 brush에 대입해라 .
            rect = QRectF(self.start,self.end)         #사각형을 시작, 끝을 한 사각형을 rect에 대입??? 
            self.scene.addRect(rect,pen,brush)         #이게 뭘까? 
        elif self.parent().drawType ==4:               #부모가 지정한 drawtype이 4면 
            brush = QBrush(self.parent().brushcolor)   #부모가 지정한 bushcolor를 Qbrush로 하고 brush에 대입?? 
            rect = QRectF(self.start, self.end)        #사각형 처음과 끝? 
            self.scene.addEllipse(rect,pen,brush)       #타원을 그리고 ?
